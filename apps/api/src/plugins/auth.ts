  ...
  async (app, options) => {
    ...
    app.decorate('authenticateTenant', async (request: FastifyRequest, reply: FastifyReply) => {
      ...
      if (presented) {
        ...
        /**
         * Two budgets, both fixed by the product rather than sold.
         *
         * The first is the per-key share of the whole authenticated surface, so one runaway
         * integration cannot spend the allowance the dashboard is using. The second is a daily
         * ceiling for the account as a whole, which is what actually bounds a patient scraper -
         * a burst limit alone does not.
         */
        await app.rateLimit(request, 'dashboardApi', `key:${principal.keyId}`);
        await app.rateLimit(request, 'publicApiDaily', `account:${principal.accountId}`);
