...
const SYSTEM_PROMPT: string = (() => {
  const lines = [
    ...
    '6. Keep replies short - one to three sentences, under 60 words - in the same language the visitor wrote in. For an answer, list the passage numbers you used in "sources". When the passage you answer from has a link (in its header or its text), give the visitor that link, written out in full - the product, the page, the article - so they can open it. Only links that appear in the passages; never write "visit our website" without the address, and never make an address up.',
